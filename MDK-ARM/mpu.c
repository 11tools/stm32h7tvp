...
/**
  ...
  * @retval None
  */
void MX_MPU_ConfigRegion(	uint8_t		Region,
														uint8_t  	DisableExec,
...
void MX_MPU_Config(uint8_t	Region ,uint8_t Mode,uint32_t Address,uint32_t Size)
{
	switch(Mode)
	{
		case MPU_Normal_WB:
				 /* 设置内存为Normal类型,禁用共享, 回写模式不带写入读取分配*/
				 MX_MPU_ConfigRegion(Region,0,MPU_FULL_ACCESS,0,Address,Size,0,1,1);	
				 break;

		case MPU_Normal_WBWARA:
				 /* 设置内存为Normal类型,禁用共享, 回写模式带写入读取分配*/
				 MX_MPU_ConfigRegion(Region,0,MPU_FULL_ACCESS,1,Address,Size,0,1,1);	
				 break;
		
		case MPU_Normal_WT:
				 /* 设置内存为Normal类型,禁用共享, 直写模式*/
				 MX_MPU_ConfigRegion(Region,0,MPU_FULL_ACCESS,0,Address,Size,0,1,0);	
				 break;		
		
		case MPU_Normal_NonCache:
				 /* 设置内存为Normal类型,禁用共享，禁用缓存模式*/
				 MX_MPU_ConfigRegion(Region,0,MPU_FULL_ACCESS,1,Address,Size,0,0,0);	
				 break;
		
		case MPU_Device_NonCache:
				 /*设置内存为Device类型,共享设置无效，禁止共享，禁用缓存模式*/
				 MX_MPU_ConfigRegion(Region,0,MPU_FULL_ACCESS,2,Address,Size,0,0,0);	
				 break;
	}
